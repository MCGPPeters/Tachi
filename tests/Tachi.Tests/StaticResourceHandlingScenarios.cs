...
namespace Tachi.Tests
{
    public class StaticResourceHandlingScenarios
    {
        ...
        private static string GetMediaType(FileSystemInfo resourceDescriptor)
        {
                ...
                (getFileDescriptors, configuredRequestPath) =>
                {
                    return next => async env =>
                    {
                        ...
                        if (requestPath.StartsWithSegments(configuredRequestPath))
                            foundResourceDescriptors = getFileDescriptors(requestPath);
                        if (foundResourceDescriptors.Any())
                        {
                            var resourceDescriptor = foundResourceDescriptors.First();

                            owinContext.Response.StatusCode = 200;
                            owinContext.Response.ContentLength = resourceDescriptor.Length;
    ...
    [Fact]
        ...
        [Fact]
        public async Task Given_a_the_request_path_is_not_the_configured_path_ignore_the_request()
        {
            GetResourceDescriptors<FileInfo> getFileDescriptorsStub = pathString => new FileInfo[0];
